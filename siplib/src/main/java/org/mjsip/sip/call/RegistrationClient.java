...
public class RegistrationClient implements TransactionClientListener, TimerListener {
	...
	public void onTransFailureResponse(TransactionClient transaction, SipMessage resp) {
		if (transaction.getTransactionMethod().equals(SipMethods.REGISTER)) {
			...
			if (code==401 && attempts<SipStack.regc_auth_attempts && resp.hasWwwAuthenticateHeader()) {
				// UAS authentication
                String remoteRealm = resp.getWwwAuthenticateHeader().getRealmParam();
                if (remoteRealm.length() > 0) realm = remoteRealm;
				attempts++;
			...
			if (code==407 && attempts<SipStack.regc_auth_attempts && resp.hasProxyAuthenticateHeader()) {
				// Proxy authentication
                String remoteRealm = resp.getProxyAuthenticateHeader().getRealmParam();
                if (remoteRealm.length() > 0) realm = remoteRealm;
				attempts++;
				SipMessage req=transaction.getRequestMessage();
